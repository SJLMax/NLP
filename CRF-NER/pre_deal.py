...
def deal(data):
    outdata = codecs.open('./data_pos.txt', 'w', 'utf-8')
...
if __name__ == '__main__':
    data=readtxt(r'./data.txt')
    all=[]
    for line in data:
        new=re.sub("<[A-Za-z1-4/]+[>]{1,5}","",line)
        all.append(new)
    print(all)
    deal(all)
    # data = readtxt('./CRF++-0.58/alldata.txt')
    # print(data)
    # train_list = data[:int(len(data) * 0.5)]
    # text_list = data[int(len(data) * 0.9):]
    # writetxt("./CRF++-0.58/train5.txt", train_list)
